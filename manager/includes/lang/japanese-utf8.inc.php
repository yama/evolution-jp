...
$_lang["allow_mgr_access_message"] = 'このオプションは管理画面へのアクセスをユーザーに許可するかしないかを選択するものです。<br />※このオプションを「いいえ」に設定すると、ユーザーは「管理画面の設定」タブの「ログイン時のリダイレクト先」か、それが空白ならサイトスタートのページへリダイレクトされます。';
...
$_lang["default"] = '既定値';
$_lang["defaultcache_message"] = '新しく作成するリソースのキャッシュ設定をデフォルトで有効にする場合は「はい」を選択してください。';
...
$_lang["defaulttemplate_message"] = '新しく作成するリソースのデフォルトテンプレートを指定します。テンプレートはリソース単位で自由に選択できます。';
$_lang["defaulttemplate_title"] = 'デフォルトテンプレート';
$_lang["defaulttemplate_logic_title"] = 'テンプレート自動設定';
$_lang["defaulttemplate_logic_general_message"] = '新しく作成するリソースのテンプレート初期設定。';
...
$_lang["file_not_saved"] = 'ファイルを更新できません。対象ファイルのパーミッションが書き込み可能か確認してください。';
...
$_lang["lock_module_msg"] = '誤操作による書き換えを防ぎます。';
$_lang["lock_msg"] = '%s さんが %s を編集中です。作業が終わるまで待つか、ロックを解除してください。';
$_lang["lock_plugin"] = 'プラグインをロック';
$_lang["lock_plugin_msg"] = '誤操作による書き換えを防ぎます。';
$_lang["lock_settings_msg"] = '%s が設定を編集中です。他のユーザーが終了するまでしばらくお待ちください。';
$_lang["lock_snippet"] = 'スニペットをロック';
$_lang["lock_snippet_msg"] = '誤操作による書き換えを防ぎます。';
$_lang["lock_template"] = 'テンプレートの編集をロック';
$_lang["lock_template_msg"] = '誤操作による書き換えを防ぎます。';
$_lang["lock_tmplvars"] = 'テンプレート変数をロック';
$_lang["lock_tmplvars_msg"] = '誤操作による書き換えを防ぎます。';
$_lang["locked"] = 'ロック中';
...
$_lang["page_data_cacheable"] = 'キャッシュ有効';
...
$_lang["plugin_management_msg"] = '<h3 style="font-weight:bold;">プラグインの管理</h3><p>プラグインは任意のシステム処理タイミング(システムイベント)に関連付けて実行します。スニペットと比べ、管理画面のカスタマイズもできるなど応用幅が広いです。</p>';
...
$_lang["resource_long_title_help"] = 'リソース変数：[*longtitle*]';
...
$_lang["resource_title_help"] = '管理画面のリソースツリーで表示されるため短めにするといいでしょう。バックスラッシュは使用できません(リソース変数：[*pagetitle*])';
...
$_lang["showing"] = '表示中';
$_lang["signupemail_message"] = 'ユーザ管理画面でユーザーアカウントを作成した時に、ログイン名とパスワードを記載したメールを送信することができます。このメールの内容は変更できます<br /><strong>参考:</strong> 下記の差し込み文字列は送信時に自動的に置き換えられます。<br /><br />[(site_name)] - サイトの名前<br />[+manager_email+] - サイト管理者のメールアドレス<br />[(site_url)] - サイトのURL<br />[+username+] - ログイン名<br />[+password+] - パスワード<br />[+fullname+] - フルネーム';
...
$_lang["tmplvars_type"] = '入力フォーム';
$_lang["tmplvars_widget"] = 'プロセッサ';
$_lang["tmplvars_widget_prop"] = 'プロセッサの設定';
$_lang["to"] = '-';
$_lang["tools"] = 'ツール';
...
$_lang["webpwdreminder_message"] = 'ウェブユーザーからパスワード変更の要求があった時に、MODXは新パスワードとアクティベーション方法を記載したメールを送信することができます。このメールの内容は変更できます。<br /><strong>参考:</strong> 下記の差し込み文字列は送信時に自動的に置き換えられます。<br /><br />[(site_name)] - サイトの名前<br />[+saddr+] - サイト管理者のメールアドレス<br />[(site_url)] - サイトのURL<br />[+username+] - ログイン名<br />[+password+] - パスワード<br />[+fullname+] - フルネーム';
$_lang["webpwdreminder_title"] = 'パスワード変更<br />リクエストメール';
$_lang["websignupemail_message"] = 'ユーザー自らがウェブユーザーアカウントを作成した時に、MODXはログイン名とパスワードを記載したメールを送信することができます。このメールの内容は変更できます。<br /><strong>参考:</strong> 下記の差し込み文字列は送信時に自動的に置き換えられます。<br /><br />[(site_name)] - サイトの名前<br />[+manager_email+] - サイト管理者のメールアドレス<br />[(site_url)] - サイトのURL<br />[+username+] - ログイン名<br />[+password+] - パスワード<br />[+fullname+] - フルネーム';
...
$_lang["which_editor_message"] = 'デフォルトで使用するリッチテキストエディターを選択してください(※<a href="http://modx.com/extras/category/richtexteditors?product=evolution" target="_blank">MODXのダウンロードページ</a>には追加でインストールできるリッチテキストエディターがあります)。';
...
$_lang['setting_cache_type_desc'] = '「標準モード」はプラグイン処理をキャッシュしません。';
$_lang['setting_conditional_get'] = '条件付きGET';
$_lang['setting_conditional_get_desc'] = '条件付きGETを有効にします。';
...
$_lang["mutate_settings.dynamic.php2"] = 'バイパスモード(高速・省メモリ)';
...
$_lang['output_filter_message'] = 'モディファイア機能を有効にします。既存のスニペットをモディファイアとして使えますが、プリセットモディファイア名とスニペット名が重複すると、そのモディファイアは意図どおりに動作しないためご注意ください。<br />※例：「date」というスニペットが存在するとdateモディファイアの機能が上書きされます。';
